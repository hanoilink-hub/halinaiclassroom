...
impl SystemAudioCapture {
    ...
    pub fn start(&self) -> Result<mpsc::Receiver<Vec<u8>>, String> {
        ...
        let enable_alac = std::env::var("MY_TRANSLATOR_ENABLE_ALAC")
            .ok()
            .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
            .unwrap_or(false);

        std::thread::spawn(move || {
            // NOTE: ALAC (ActivateAudioInterfaceAsync + PROPVARIANT blob) is fragile on some
            // Windows setups and can crash the process. Default to the legacy loopback path.
            // To opt-in to ALAC (excludes own process audio), set env MY_TRANSLATOR_ENABLE_ALAC=1.
            if enable_alac {
                match start_app_loopback(own_pid, sender.clone(), is_capturing.clone()) {
                    Ok(()) => return,
                    Err(e) => {
                        eprintln!(
                            "[wasapi] Application Loopback (ALAC) failed: {}. Falling back to legacy loopback.",
                            e
                        );
                    }
                }
