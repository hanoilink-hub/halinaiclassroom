...
## HaLin Classroom Capture (integration)

This repo can also act as a **teacher desktop client** for HaLin:

- Capture **System / Mic / Both** audio on the teacher machine
- Run STT (via Soniox real-time engine already used by the app)
- Buffer transcript locally and send **chunks every 2–10 seconds** to HaLin
- End session → finalize → HaLin worker scores the lesson using the teacher profile

### Backend endpoints used

- `POST /api/v1/training/jobs/live-session` (create a `live_capture` job)
- `POST /api/v1/training/jobs/{job_id}/transcript-chunks` (idempotent append by `chunk_seq`)
- `POST /api/v1/training/jobs/{job_id}/finalize-live` (merge chunks → write `live_session.json` → enqueue)
- Fallback recovery: `POST /api/v1/training/jobs/from-transcript` (send full transcript if finalize fails)

### Desktop settings

In Settings → Translation → **HaLin Classroom Capture**:

- **Base URL**: HaLin API, e.g. `http://127.0.0.1:8000`
- **Desktop API Token**: sent as `Authorization: Bearer ...` if configured on backend
- **Chunk seconds**: recommend 5s for classrooms
- **Session profile preset**: `lesson_type`, `level`, `topic`, `expected_interaction_mode`

### Retry + recovery

- The desktop app keeps an on-device queue in `localStorage` keyed by `job_id` and retries automatically.
- If finalize fails, you can use **fallback recovery** (send a full merged transcript) from the buffered chunks.
